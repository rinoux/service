...
function drawCld(SY,SM) {
   ...
   for(i=0;i<42;i++) {
	  ...
	  $(sObj).removeClass("todaydiv");	
	  //$(lObj).removeClass("festival");	
	  
	  //console.log(sObj);
	  //console.log(lObj);
	  
      //sObj.className = 'circle';
      sD = i - cld.firstWeek;
      if(sD>-1 && sD<cld.length) { //日期内
         sObj.innerHTML = sD+1;
		 if(cld[sD].isToday){ 
		 	sObj.style.color = '#FFFFFF';
			//$(sObj).toggleClass("todaydiv");	
			
			
		 } //今日颜色
		 
		 else{sObj.style.color = '#ffffff';}
...
function initial() {
	//YEARSPAN1.innerHTML = tY + " 年";	//年
	//MONTHSPAN1.innerHTML = tM + 1 + " 月";	//月
	YEARSPAN.innerHTML = tY + " 年";	//年
	MONTHSPAN.innerHTML = tM + 1 + " 月";	//月
//	GZ.innerHTML = '【'+Animals[(tY-4)%12]+'】';	//生肖
	drawCld(tY,tM);
}